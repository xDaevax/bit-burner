export class Container {
	...
	create(headerText) {
		...
		script.setAttribute('src', 'https://cdn.jsdelivr.net/npm/feather-icons/dist/feather.min.js');
		script.onload = () => {
			feather.replace();
		};
	...
	createHeaderText(headerText) {
		const node = this.#dom.createElement('p');
	...
	createElement(name, id, classList) {
		const element = this.#dom.createElement(name);
	...
	render(headerText) {
		...
		this.dragElement(this.findElementById(this.id), this.#dom);
	}
}